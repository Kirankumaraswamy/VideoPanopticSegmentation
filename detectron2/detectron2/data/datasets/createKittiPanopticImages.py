...
kitty_to_city = {
    "0": 26, "1": 33, "2": 32, "3": 27, "4": 0, "5": 24,"6": 25, "7": 25,"8": 7,"9": 9,"10": 8,"11": 6,"12": 11,"13": 13,"14": 21,"15": 21,"16": 22,"17": 17,"18": 20,"255": 0
}

id2label        = { label.id      : label for label in labels           }

# The main method
def convert2panoptic(kittiPath=None, outputFolder=None, useTrainId=False, setNames=["train", "val", "test"]):
    # Where to look for Cityscapes
    if kittiPath is None:
        if 'KITTI_DATASET' in os.environ:
            kittiPath = os.environ['KITTI_DATASET']
        else:
            kittiPath = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../../projects/Panoptic-DeepLab', '..')
        kittiPath = os.path.join(kittiPath, "video_sequence")

    if outputFolder is None:
        outputFolder = kittiPath
    ...
    for setName in setNames:
        ...
        searchFine   = os.path.join(kittiPath, setName, "*_instance.png")
        search_folder = os.path.join(kittiPath, setName)
        # search files
        ...
        outputBaseFile = "kitti_panoptic_{}{}".format(setName, trainIfSuffix)
        ...
        for progress, f in enumerate(files):
            ...
            imageId = fileName.replace("_gtFine_instance.png", "")
            inputFileName = fileName.replace("_gtFine_instance.png", "_leftImg8bit.png")
            semanticFile_name = fileName.replace("instance.png", "class.png")
            outputFileName = fileName.replace("_gtFine_instance.png", "_panoptic.png")
            ...
            semanticFormat = np.array(Image.open(os.path.join(search_folder, semanticFile_name)))
            ...
            for segmentId in segmentIds:
                ...
                # for stuff objects
                if segmentId == 0:
                    semanticIds = np.unique(semanticFormat[mask])
                    for semanticId in semanticIds:
                        semantic_mask = semanticFormat == semanticId
                        city_semantic_id = kitty_to_city[str(semanticId)]
                        semanticId = city_semantic_id
                    ...
                    semantic = semanticFormat[mask]
                    semanticId = np.argmax(np.bincount(semantic))
                    # kitty to cityscapes conversion
                    semanticId = kitty_to_city[str(semanticId)]


                    labelInfo = id2label[semanticId]
                    categoryId = labelInfo.trainId if useTrainId else labelInfo.id

                    segmentId = categoryId * 1000 + segmentId

                    isCrowd = 0

                    # color = [((categoryId+1)*1000 + segmentId)  % 256, ((categoryId+1)*1000 + segmentId) // 256, ((categoryId+1)*1000 + segmentId) // 256 // 256]
                    color = [segmentId % 256, segmentId // 256, segmentId // 256 // 256]
                    pan_format[mask] = color
...
def main():
    ...
    convert2panoptic(args.kittiPath, args.outputFolder, args.useTrainId, args.setNames)
